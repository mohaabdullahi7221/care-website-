...
const heroSlides = [
  ...
  "/school/downloads/stem-engineering-team.jpeg",
  "/school/students-mixed.jpg",
  "/school/students-outdoor-girls.jpg",
  "/school/students-indoor-1.jpg",
  "/school/students-indoor-2.jpg",
  "/school/students-indoor-3.jpg",
...
function Home({ go }) {
  ...
  useEffect(() => {
    const timer = setInterval(() => setSlide((current) => (current + 1) % heroSlides.length), 3000);
  ...
  return <>
    <section className="hero">{heroSlides.map((image, index)=><div key={image} className={`hero-bg ${slide===index?"active":""}`} style={{backgroundImage:`linear-gradient(90deg,rgba(4,30,65,.95) 0%,rgba(4,30,65,.74) 48%,rgba(4,30,65,.12) 100%),url('${image}')`}}></div>)}<div className="wrap hero-in"><div className="hero-copy"><div className="pill">Tan iyo 2010 • Wadajir, Muqdisho</div><h1>Ku Soo Dhawoow <em>CARE</em> Primary and Secondary School</h1><p>Waxbarasho Tayo Leh, Tarbiyad Wanaagsan iyo Mustaqbal Iftiimaya</p><div className="actions"><Link to="/nagu-saabsan" onGo={go} className="btn gold">Wax Badan Ka Ogow <b>→</b></Link><Link to="/xiriir" onGo={go} className="btn ghost">Nala Soo Xiriir</Link></div></div><div className="hero-note"><span>Jiilka Berri</span><strong>Halkan ayuu ka bilaabmaa.</strong></div><div className="hero-dots" aria-label="Dooro sawirka weyn">{heroSlides.map((_,index)=><button key={index} className={slide===index?"active":""} onClick={()=>setSlide(index)} aria-label={`Sawirka ${index+1}`} aria-current={slide===index?"true":undefined}></button>)}</div></div></section>
    <Stats />
