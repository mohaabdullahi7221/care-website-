...
export function Navbar({ path, go }) {
  ...
  return <>
    <div className="topbar"><div className="wrap topbar-in"><span>📍 Jidka Afgooye, Wadajir, Muqdisho</span></div></div>
...
export function VideoHighlight() {
  ...
  function start() { setPlay(true); }
  return (
    <section className="section soft video-section">
      <div className="wrap">
        <SectionTitle eyebrow="MUUQAALKA DUGSIGA" title="Daawo Nolosha CARE" text="Riix ama dul joog si aad u daawatid muuqaalka dugsiga." />
        <div className="video-frame" onClick={start} onMouseEnter={start} role="button" tabIndex={0} onKeyDown={(e) => { if (e.key === "Enter" || e.key === " ") { e.preventDefault(); start(); } }} aria-label="Daawo muuqaalka CARE School">
          {play ? (
            <iframe title="Muuqaalka CARE School" src="https://www.youtube.com/embed/ZbxOuJXYcwY?autoplay=1&rel=0" allow="autoplay; encrypted-media; picture-in-picture" allowFullScreen />
          ) : (
            <>
              <img src="https://img.youtube.com/vi/ZbxOuJXYcwY/hqdefault.jpg" alt="Muuqaalka CARE School" />
